...
const CLASS_COLORS = { hiit: '#FF4444', yoga: '#9B59B6', crossfit: '#FF6B00', cycling: '#39FF14', boxing: '#E74C3C', strength: '#3498DB', pilates: '#1ABC9C', cardio: '#F39C12', open_gym: '#888888' };
...
export default function BookPage() {
  ...
  const filtered = classes.filter(cls => {
    ...
    const dayMatch = cls.schedule?.some(s => s.dayOfWeek === selectedDate.getDay());
    ...
    <div className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Category Filter */}
          <div>
            <h3 className="text-sm font-semibold text-white/50 uppercase tracking-widest mb-4">Class Type</h3>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="card p-12 text-center">
                ...
                <p className="text-white/50">No classes available for this selection.</p>
                <p className="text-white/30 text-sm mt-1">Try a different day or category.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {/* Open Gym Option */}
                {(selectedCategory === 'All' || selectedCategory === 'Open Gym') && (
                  <button
                    ...
                    className={`w-full text-left card p-5 transition-all duration-200 border ${
                      !selectedClass ? 'border-neon/40 bg-neon/5' : 'hover:border-white/10'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-white/40 text-sm">Self-directed • Any time • Main Floor</p>
                      </div>
                ...
                {filtered.map(cls => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left card p-5 transition-all duration-200 border ${
                        isSelected ? 'border-neon/40 bg-neon/5' : 'hover:border-white/10'
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            <span className="text-xs text-white/30 uppercase">{cls.difficulty}</span>
                          </div>
                          <h4 className="font-bold text-sm mb-0.5">{cls.name}</h4>
                          <p className="text-white/40 text-xs">with {cls.trainer.name}</p>
